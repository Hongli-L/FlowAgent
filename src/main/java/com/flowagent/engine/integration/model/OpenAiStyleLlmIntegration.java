...
/**
 * OpenAI-style LLM integration
 * Compatible with any OpenAI-compatible model service (via base-url configuration)
 */
@Slf4j
@Component
public class OpenAiStyleLlmIntegration {
    // Use regex to extract the domain and path parts separately
    ...
    private OpenAiApi initClient(String key, String apiUrl) {
        ...
        if (matcher.matches()) {
            baseUrl = matcher.group(1);  // Domain part
            basePath = matcher.group(2);           // Path part (may be null)
        } else {
            // If no match, fall back to the original baseUrl
            baseUrl = apiUrl;
        }

        if (apiUrl.contains(".aliyuncs.com")) {
            // Alibaba Cloud model service requires a specific compatible-mode base URL
            baseUrl = apiUrl + "/compatible-mode";
            basePath = null;
        }
    ...
    private Prompt buildPrompt(LlmReqBo req) {
        ...
        if (!CollectionUtils.isEmpty(req.getHistory())) {
            // If chat history is not empty, build conversation history
            for (LlmChatHistory.ChatItem item : req.getHistory()) {
                for (LlmChatHistory.ChatMessage message : item.userInputs()) {
                    if (message.role() == MsgTypeEnum.SYSTEM) {
                        // Note: when building LLM conversation history, existing system messages are skipped to avoid conflicts with the current system message
                        continue;
                    } else {
    ...
    private OpenAiChatOptions buildChatOption(LlmReqBo req) {
        ...
        if (req.getExtraParams() != null) {
            // Handle extra parameters in a more concise way
