package com.flowagent.engine.domain.callbacks;

import com.flowagent.common.enums.ChatStatusEnum;
...
public class ChatCallBacks {
    ...
    private double getNodeProgress(String currentExecuteNodeId) {
        // Simplified implementation, no dependency on Chains
        return 0.0;
    }
    ...
    private void putFrameIntoQueue(String nodeId, LLMGenerate resp, String finishReason) {
        String nodeType = nodeId.split(":")[0];
        if (false && (nodeType.equals(NodeTypeEnum.LLM.getValue()) || nodeType.equals(NodeTypeEnum.END.getValue()) || "WorkflowEnd".equals(nodeId)) ) {
